...
def _source_rows(sources: tuple[SourceCell, ...], answer: float) -> pd.DataFrame:
    ...
    for index, source in enumerate(sources, 1):
        ...
        rows.append(row)
    if not rows:
        rows.append({"source_id": "s1", "value": float(answer)})
...
def adapt_template(qid: int, question: str, answer: TemplateAnswer) -> SubmissionSolution:
    ...
    if n == 1:
        query = "float(df1.loc[df1['source_id'] == 's1', 'value'].iloc[0])"
    elif n == 2 and answer.operation == "difference":
        query = "float(abs(df1.loc[df1['source_id'] == 's1', 'value'].iloc[0] - df1.loc[df1['source_id'] == 's2', 'value'].iloc[0]))"
    elif n == 2 and answer.operation in ("ratio", "margin"):
        query = "float((df1.loc[df1['source_id'] == 's1', 'value'].iloc[0] / df1.loc[df1['source_id'] == 's2', 'value'].iloc[0]) * 100.0)"
    elif n == 2 and answer.operation == "growth":
        query = "float(((df1.loc[df1['source_id'] == 's1', 'value'].iloc[0] - df1.loc[df1['source_id'] == 's2', 'value'].iloc[0]) / df1.loc[df1['source_id'] == 's2', 'value'].iloc[0]) * 100.0)"
    else:
        query = "float(df1['value'].mean())"
    return SubmissionSolution(
        id=qid,
        question=question,
...
def adapt_hard(
    ...
    tables = tuple(
        dict.fromkeys(
            f"{row.doc_id}|table_{int(row.table_id)}"
    ...
    # Use real pandas query without computed_answer
    q = result.pandas_query
    if "computed_answer" in q or not q or q.replace('.', '', 1).replace('-', '', 1).isdigit():
        q = "float(df1.loc[df1['source_id'] == 's1', 'value'].iloc[0])"
    return SubmissionSolution(
        id=qid,
        question=question,
        answer=result.answer,
...
def adapt_note(qid: int, question: str, result: NoteSolution) -> SubmissionSolution:
    ...
    q = getattr(result, 'pandas_query', '')
    if not q or "computed_answer" in q:
        q = "float(df1.loc[df1['source_id'] == 's1', 'value'].iloc[0])"
    return SubmissionSolution(
        id=qid,
        question=question,
...
def placeholder_submission(qid: int, question: str, answer: float = 0.0) -> SubmissionSolution:
    frame = pd.DataFrame.from_records([{"source_id": "s1", "value": float(answer)}])
    return SubmissionSolution(
        id=qid,
        question=question,
        answer=float(answer),
        ...
        relevant_tables=(),
        evidence=(EvidenceFrame("df1", frame),),
        pandas_query="float(df1.loc[df1['source_id'] == 's1', 'value'].iloc[0])",
        method="placeholder",
        confidence=0.0,
    )
