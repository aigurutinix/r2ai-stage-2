...
def source_scale(text: object) -> float:
    ...
    if (
        ...
        or "ty vnd" in folded
        or folded == "ty"
        or folded.endswith(" ty")
        or "dvt ty" in folded
        or "don vi tinh ty" in folded
        or "don vi ty" in folded
        or "(ty" in folded
    ):
    ...
    if (
        ...
        or "dvt trieu" in folded
        or "don vi tinh trieu" in folded
        or "don vi trieu" in folded
        or folded == "trieu"
        or folded.endswith(" trieu")
        or "(trieu" in folded
    ):
        return 1_000_000.0
    if (
        any(unit in folded for unit in ("nghin dong", "ngan dong", "nghin vnd", "ngan vnd"))
        or "dvt nghin" in folded
        or "don vi tinh nghin" in folded
        or "don vi nghin" in folded
        or "dvt ngan" in folded
        or "don vi tinh ngan" in folded
        or folded in ("nghin", "ngan")
        or folded.endswith(" nghin")
        or folded.endswith(" ngan")
        or "(nghin" in folded
        or "(ngan" in folded
    ):
        return 1_000.0
...
def to_base_vnd(value: float, scale: float) -> float:
    """Normalize numeric value to base VND currency."""
    return float(value) * float(scale)


def from_base_vnd(base_val: float, target_scale: float) -> float:
    """Convert base VND value to target scale (e.g. billion, million)."""
    return float(base_val) / float(target_scale)
