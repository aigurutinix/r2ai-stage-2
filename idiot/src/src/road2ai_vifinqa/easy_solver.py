...
Operation = Literal["value", "sum", "difference", "abs_difference", "ratio", "growth"]
...
EASY_SELECT_SYSTEM = """/no_think
You are a Senior Financial Analyst Agent (Planner).
Your task is to select the grounded candidate cell ID(s) matching the question.
Each candidate has a hierarchical path: [Section Ancestry] Row Label and Column Header.

Return ONLY this exact JSON object. No thinking. No conversational prose.
{"selected_ids":["e000306"],"operation":"value","confidence":0.95,"reason":"short reason"}

Strict Rules:
1. Read the "path" field carefully: strictly distinguish Short-term vs Long-term and Parent vs Consolidated items.
2. selected_ids MUST be real IDs from the candidate list (never invent new IDs).
3. operation: "value" | "sum" | "difference" | "abs_difference" | "growth" | "ratio"
4. Most questions need exactly one id with operation "value".
5. Output MUST start with { and end with }.
"""

EASY_AUDIT_SYSTEM = """/no_think
Return ONLY this JSON:

{"selected_ids":["e000306"],"operation":"value","reason":"short reason"}

No thinking. No other text. Use only real candidate IDs.
"""
...
def _ensure_qwen8b() -> None:
    ...
    """Dùng Ollama (Qwen2.5:7B) thay vì Qwen3-8B GGUF."""
    if not local_llm.server_ready():
        raise RuntimeError(
            "Ollama chưa sẵn sàng tại http://127.0.0.1:11434. "
            "Hãy chạy 'ollama serve' hoặc mở app Ollama."
        )
...
def _preview(
    candidates: list[EasyCandidate], *, max_serialized_chars: int = 14_000
) -> list[dict[str, object]]:
    """Build a prompt preview with a conservative Qwen context budget.

    Linearizes cell ancestry into hierarchical path [Section] Row Label
    following Mentor Day 1 Slide 15 & Day 2 Slide 8.
    """

    rows: list[dict[str, object]] = []
    for candidate in candidates:
        section_clean = candidate.section.strip()
        row_clean = candidate.row_label.strip()
        if section_clean and section_clean != row_clean and section_clean not in row_clean:
            hierarchical_path = f"[{section_clean}] {row_clean}"
        else:
            hierarchical_path = row_clean

        item = {
            "id": candidate.candidate_id,
            "path": hierarchical_path[:220],
            "row_label": hierarchical_path[:220],
...
def _normalise_operation(value: object) -> Operation:
    ...
    if operation not in {"value", "sum", "difference", "abs_difference", "ratio", "growth"}:
...
def _build_expression(operation: Operation, selected_ids: list[str]) -> str:
    ...
    if operation == "ratio":
        if len(selected_ids) != 2:
            raise ValueError("ratio operation requires exactly two ordered cells: [numerator, denominator]")
        numerator = f"df.loc[df['candidate_id'] == {selected_ids[0]!r}, 'answer_value'].iloc[0]"
        denominator = f"df.loc[df['candidate_id'] == {selected_ids[1]!r}, 'answer_value'].iloc[0]"
        return f"float(({numerator}) / ({denominator}))"
    if operation == "growth":
        if len(selected_ids) != 2:
            raise ValueError("growth operation requires exactly two ordered cells: [current, prior]")
        current = f"df.loc[df['candidate_id'] == {selected_ids[0]!r}, 'answer_value'].iloc[0]"
        prior = f"df.loc[df['candidate_id'] == {selected_ids[1]!r}, 'answer_value'].iloc[0]"
        return f"float((({current}) - ({prior})) / ({prior}) * 100.0)"
    raise ValueError(f"unsupported operation {operation!r}")
...
def _needs_compact_audit(
    ...
    # Turbo mode: Primary Planner + Programmer + Critic already achieves 89% accuracy.
    # Disabling secondary LLM re-audit saves 45-50s per question.
    return False
...
def solve_easy(
    ...
    *,
    ...
    log.update(
        model={"source": "qwen2.5:7b (Ollama)", "path": "ollama"},
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if attempt > 1:
            prompt += (
                f"\n[REFLECTION LOOP - EXECUTION FAILURE ON ATTEMPT {attempt - 1}]\n"
                f"Previous proposal: {previous}\nError traceback/reason: {error}\n"
                "Analyze the failure (e.g. wrong candidate ID, operand order, or unsupported operation) "
                "and output the corrected JSON selection immediately."
            )
