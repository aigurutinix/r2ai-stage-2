...
def hierarchical_row_path(row_label: str, section: str) -> str:
    """Return linearized hierarchical path: [Section] -> Row Label.

    Implements Hierarchical Schema Encoding: c_{i,j} = (RowPath, ColPath, Value)
    following Mentor Day 1 Slide 15 & Day 2 Slide 8.
    """
    r_clean = clean_text(row_label)
    s_clean = clean_text(section)
    if not s_clean or s_clean == r_clean:
        return r_clean
    if s_clean in r_clean:
        return r_clean
    return f"[{s_clean}] {r_clean}"
