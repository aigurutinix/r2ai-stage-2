...
def financial_sanity_check(solution: SubmissionSolution, question: str) -> str | None:
    """Validate economic sanity of financial solutions.

    Detects unit scale mismatches (>1000% ratio), impossible negative quantities
    (negative revenue, assets, cash), and checks for undirected difference signs.
    """
    folded = fold_text(question)
    try:
        ans = float(solution.answer)
    except (ValueError, TypeError):
        return "non_numeric_answer"

    # 1. Ratio & Growth rate sanity bounds
    is_ratio_q = any(
        kw in folded
        for kw in ("ty le", "ty trong", "bien loi nhuan", "roa", "roe", "tang truong", "bien dong bao nhieu %")
    )
    if is_ratio_q:
        # Ratios > 1000% or < -100% strongly indicate an order-of-magnitude scale mismatch between periods
        if ans > 1000.0 or ans < -100.0:
            return f"abnormal_ratio_scale: {ans:.2f}% exceeds acceptable [-100%, 1000%] range"

    # 2. Inherently non-negative accounting metrics
    non_negative_metrics = (
        ("doanh thu thuan", "doanh thu"),
        ("tong tai san", "tai san"),
        ("tien va tuong duong tien", "tien mat"),
        ("hang ton kho",),
        ("von dieu le",),
    )
    for metric_keywords in non_negative_metrics:
        if any(kw in folded for kw in metric_keywords):
            # If the question asks for a single point-in-time value (not a difference/growth), answer cannot be negative
            is_delta = any(kw in folded for kw in ("tang", "giam", "chenh lech", "bien dong", "thay doi"))
            if not is_delta and ans < 0:
                return f"negative_value_for_positive_metric: {metric_keywords[0]} = {ans}"

    # 3. Undirected differences should not be negative
    if any(kw in folded for kw in ("chenh lech", "bien dong")) and "giam" not in folded and ans < 0:
        return "negative_undirected_difference"

    return None


def _apply_financial_critic(
    solution: SubmissionSolution,
    question_id: int,
    question: str,
    route: str,
) -> SubmissionSolution:
    """Independent Critic / Verifier Agent (Agent 5).

    Checks accounting sanity rules and executes reflection corrections.
    """
    issue = financial_sanity_check(solution, question)
    if issue is None:
        return solution
    ...
    # Reflection Correction 1: Undirected difference with negative sign
    if any(kw in folded for kw in ("chenh lech", "bien dong")) and "giam" not in folded and ans < 0:
        new_ans = abs(ans)
        query = solution.pandas_query
        new_query = f"abs({query})" if not query.startswith("abs(") else query
        return replace(
            solution,
            answer=new_ans,
            pandas_query=new_query,
            method=f"{solution.method}:critic_abs",
        )

    # Reflection Correction 2: Scale mismatch in growth/ratio questions (>1000%)
    if "abnormal_ratio_scale" in issue and abs(ans) >= 1000.0:
        # Check if dividing by 1000 (thousand vs million VND unit shift) resolves the anomaly
        scaled_ans = ans / 1000.0
        if -100.0 <= scaled_ans <= 1000.0:
            query = solution.pandas_query
            new_query = f"({query}) / 1000.0"
            return replace(
                solution,
                answer=scaled_ans,
                pandas_query=new_query,
                method=f"{solution.method}:critic_scaled",
            )
        ...
        for position, question_id in enumerate(selected, 1):
            ...
            try:
                if solution is None:
                    ...
                    solution = _apply_financial_critic(
                        solution,
                        question_id,
                        question,
                        route,
