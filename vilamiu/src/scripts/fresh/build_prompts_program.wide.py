...
LABEL_CHARS = 58
CELL_CHARS = 22
# Column headings carry the period and the unit — `Ngày 31 tháng 12 năm 2020 Triệu đồng`.
# At 22 characters, 28% of them were cut, and what survived was `Từ ngày 15/01/2015đến `
# or `Chênh lệch giữa năm 20`: the year gone from a heading in a task where every question
# names a year. Headings get their own, much longer, budget.
HEADER_CHARS = 72
BUDGET = 24000
MAX_TABLES = 9
...
def render(ref: str, context: str, grid: list[list[str]]) -> str:
    ...
    lines.append("  ".join(
        f"c{i}={str(header[i]).strip()[:LABEL_CHARS if i == 0 else HEADER_CHARS]}"
